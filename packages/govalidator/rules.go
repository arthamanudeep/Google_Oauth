...
func init() {
	...
	// Alpha check if provided field contains valid letters
	AddCustomRule("alpha_noSpace", func(field string, vlaue string, message string, value interface{}) error {
		str := toString(value)
		err := fmt.Errorf("The %s should not contain spaces at start and accepts only _,.,- as special characters", field)
		if message != "" {
			err = errors.New(message)
		}
		if !isAlphaNoSpaceAtStart(str) {
			return err
		}
		return nil
	})
	...
	// AlphaSpecial check if provided field contains valid letters, numbers, underscore and dash
	AddCustomRule("alpha_special", func(field string, vlaue string, message string, value interface{}) error {
		str := toString(value)
		err := fmt.Errorf("The %s may only contain letters, numbers, dashes, space, underscore, and dot", field)
		if message != "" {
			err = errors.New(message)
		}
		if !isAlphaSpecial(str) {
			return err
		}
		return nil
	})

	// AlphaSpace check if provided field contains valid letters, numbers, underscore and dash
	...
	// AlphaWithSpace check if provided field contains valid letters, numbers, underscore and dash
	AddCustomRule("alpha_with_space", func(field string, vlaue string, message string, value interface{}) error {
		str := toString(value)
		err := fmt.Errorf("The %s may only contain letters and space", field)
		if message != "" {
			err = errors.New(message)
		}
		if !isAlphaWithSpace(str) {
			return err
		}
		return nil
	})
	...
	AddCustomRule("exists", func(field string, rule string, message string, value interface{}) error {
		...
		message = "given " + queryField + " does not exist in " + strings.ToLower(recordName) + "s"

		// var result interface{}
		record := GlobalV.Opts.Records[recordName]

		var exists bool

		if strings.Contains(queryField, "_id") {
			if bson.IsObjectIdHex(toString(strValue)) {
				exists = record.Exists(bson.M{queryField: bson.ObjectIdHex(strValue)})
			} else {
				return errors.New("not a valid mongo object_id")
			}
			//count, err2 = record.Find(bson.M{queryField: bson.ObjectIdHex(strValue)}).Count()
		} else {
			exists = record.Exists(bson.M{queryField: strValue})
		}
	...
	AddCustomRule("mongoUnique", func(field string, rule string, message string, value interface{}) error {
		// field -- email
		// rule -- "mongoUnique:User|ignore:_id,12234234"
		// message --
		// value -- 2@tericsoft.com

		var dbQueryField string
		var ignoreQueryField string
		var ignoreQueryValue string
		var ignoreDBValue string

		// if ignore block exists -- then rewrite rule string as 1st part
		if strings.Contains(rule, "|") {
			bits := strings.Split(rule, "|")
			rule = bits[0]
			ignoreBlock := bits[1]
			ignoreBits := strings.Split(strings.TrimPrefix(ignoreBlock, "ignore:"), ",")
			fmt.Println("Log -- ignoreBits", ignoreBits, ignoreBlock)

			if len(ignoreBits) != 2 {
				panic(errInvalidArgument)
			}

			ignoreQueryField = ignoreBits[0]

			ignorePayloadField := ignoreBits[1]
			if GlobalV.Opts.Body[ignorePayloadField] != nil {
				ignoreQueryValue = GlobalV.Opts.Body[ignorePayloadField].(string)
			} else {
				return errors.New("cannot get value of ignored field")
			}
		}

		recordBits := strings.Split(strings.TrimPrefix(rule, "mongoUnique:"), ",")
		if len(recordBits) == 0 {
			panic(errInvalidArgument)
		}
		recordName := recordBits[0]
		dbQueryValue := toString(value)

		if len(recordBits) > 1 {
			dbQueryField = recordBits[1]
		} else {
			dbQueryField = field
		}

		message = "given " + dbQueryField + " already exists in " + strings.ToLower(recordName) + "s"

		record := GlobalV.Opts.Records[recordName]

		var dbRecord interface{}
		var exists bool
		// perform DB query
		if strings.Contains(dbQueryField, "_id") {
			// check with BSON filter
			exists = record.Exists(bson.M{dbQueryField: bson.ObjectIdHex(dbQueryValue)})
			dbRecord, _ = record.View(bson.M{dbQueryField: bson.ObjectIdHex(dbQueryValue)})
		} else {
			// check with string filter
			exists = record.Exists(bson.M{dbQueryField: dbQueryValue})
			dbRecord, _ = record.View(bson.M{dbQueryField: dbQueryValue})
		}

		if !exists {
			// got value not found error -- so unique
			return nil
		}

		// if reached here -- then DB record exists -- check if it is ignorable value

		if ignoreQueryField == "" {
			// no ignore part -- so raise error since value exists
			return errors.New(message)
		}

		dbResult := dbRecord.(bson.M)

		if strings.Contains(ignoreQueryField, "_id") {
			// get actual value in DB which need to be ignored
			if dbResult[ignoreQueryField] != nil {
				ignoreDBValue = dbResult[ignoreQueryField].(bson.ObjectId).Hex()
			}

		} else {
			if dbResult[ignoreQueryField] != nil {
				ignoreDBValue = dbResult[ignoreQueryField].(string)
			}
		}

		// check if user's ignore value and DB record value matches
		if ignoreDBValue == ignoreQueryValue {
