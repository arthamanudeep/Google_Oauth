...
type IValidationDBChecker interface {
	Exists(interface{}) bool
	View(interface{}) (interface{}, error)
}
...
type (
	...
	Options struct {
		...
		Records         map[string]IValidationDBChecker
	...
	for field, rules := range v.Opts.Rules {
		...
		for _, rule := range rules {
			...
			if strings.HasPrefix(field, "file:") {
				...
				// reqVal := strings.TrimSpace(v.Opts.Request.Form.View(field))
				// reqVal := "value"
